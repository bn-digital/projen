...
export class BrandNewProject extends Project {
  ...
  constructor(options: BrandNewProjectOptions) {
    ...
    if (!options.parent) {
      this.yarn = new node.Yarn(this.root, {private: true});
    }
    ...
    if (options.docker) this.docker = new docker.Docker(this, options.docker);
  ...
  protected static _withDefaults<T extends BrandNewProjectOptions>(options: T): T {
    ...
    return {
      ...
      packageName: packageName ?? (parent ? [`@${parent?.name}`, name].map((it) => BrandNewProject._slugify(it)).join("/") : BrandNewProject._slugify(name)),
    };
  }
